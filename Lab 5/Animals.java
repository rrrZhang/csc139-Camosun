 ...
 * the method display and toString, and implements the Comparable interface
 *
...
public class Animals implements Comparable<Animals> {
    ...
    /**
     * compare animals by their name in alphabetical order
     *
     * @return integral positive or negative
     */
    @Override
    public int compareTo(Animals o) {
        if (age < o.age) {//if age < input new age
            return -1;//no need to change, return negative value
        } else if (age > o.age) {//if age > input new age
            return 1;//need to change, return positive value
        } else {//if age = input new age
            return 0;//no need to change, return 0
        }
    }
}
