...
public class AnimalGUI extends JFrame implements ActionListener {
    ...
    private LinkedQueue<Animals> animalQueue = new LinkedQueue<Animals>();
    ...
    public void actionPerformed(ActionEvent ev) {
        ...
        if (object == addReptileButton) {//if buttom clicks the reptile button
            ...
            try {
                ...
                animalQueue.enqueue(r);//enqueue the r to the queue
            ...
            try {
                ...
                animalQueue.enqueue(m);//enqueue the m to the queue
        ...
        } else if (object == removeAnimalButton) {//if buttom clicks remove button

            try {
                animalQueue.dequeue();//dequeue the animal from the queue

            } catch (EmptyQueueException ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage(),
                        "No Animals in the queue",
                        JOptionPane.ERROR_MESSAGE);//try_catch check if queue empty
            }
        } else if (object == nextAnimalButton) {//if buttom clicks the next button

            try {
                Animals animal = animalQueue.first();
                //object animal of Animals class  = queue's first element
                verifyArea.append(animal.toString()); //display queue's first element
            ...
            verifyArea.append("size: " + animalQueue.size() + "\n");//display size
            verifyArea.append(animalQueue.toString());//display all animals in queue
        }
    }
